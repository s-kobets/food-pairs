...
interface LinkType {
  source: NodeType;
  target: NodeType;
  type: 'category-link' | 'combination-link';
  value: number;
  rating: number;
  description?: string;
  item1_type?: string;
...
const ForceGraph:  React.FC<ForceGraphProps> = ({categories = [], foods = [], combinations = [], searchTerm = ''}) => {
  if (![categories, foods, combinations].every(Boolean)) return <div className="p-4">Loading...</div>
  ...
  const getItemName = (type: LinkType['item1_type'], id: LinkType['item1_id']) => {
    if (type === 'food') {
  ...
  useEffect(() => {
    ...
    const combinationLinks: LinkType[] = [];
    ...
    const filteredLinks = links.filter(link =>
      nodesToShow.includes(link.source) && nodesToShow.includes(link.target)
    );
    ...
    // Define simulation
    const simulation = d3.forceSimulation<NodeType>(nodesToShow)
      .force("link", d3.forceLink<NodeType, LinkType>(filteredLinks)
        .id((d: NodeType) => d.id + '-' + d.type)
        .distance((d: LinkType) => {
          if (d.type === 'category-link') return 160;
          if (d.rating <= 2) return 400;
          if (d.rating >= 4) return 100;
          return 300;
        }))
      .force("charge", d3.forceManyBody<NodeType>().strength((d) => {
        return d.type === 'category' ? -600 : -200;
      }))
      .force("center", d3.forceCenter(width / 2, height / 2))
      .force("collide", d3.forceCollide<NodeType>().radius((d) => d.radius * 1.2));
    ...
    const link = g.append("g")
      ...
      .attr("class", d => d.type)
      .attr("stroke-width", d => {
        if (d.type === 'category-link') return 1;
        return 1 + Math.abs(d.value - 3); // Thicker lines for extreme ratings (both good and bad)
      })
      .attr("stroke", d => {
        if (d.type === 'category-link') return "#ccc";
        // Red for bad (1-2), yellow for mediocre (3), green for good (4-5)
        if (d.rating <= 2) return "#E53935"; // Red for bad combinations
        if (d.rating === 3) return "#FFC107"; // Yellow for average
    ...
    const node = g.append("g")
      ...
      .on("mouseover", (event: MouseEvent, d: NodeType) => {
        tooltipDiv.transition()
          .duration(200)
          .style("opacity", 0.9);
        ...
        const position = event instanceof MouseEvent
          ...
          : { x: (event as TouchEvent).touches[0].pageX, y: (event as TouchEvent).touches[0].pageY };
        ...
        link
          .attr("stroke-opacity", l => {
            if (l.source.id === d.id && l.source.type === d.type ||
              l.target.id === d.id && l.target.type === d.type) return 1;
            return 0.1;
          })
          .attr("stroke-width", l => {
            if (l.source.id === d.id && l.source.type === d.type ||
              l.target.id === d.id && l.target.type === d.type) {
              if (l.type === 'category-link') return 2;
        ...
        node.selectAll("circle")
          // @ts-ignore
          .attr("opacity", (n: NodeType) => {
            const isConnected = links.some(l =>
              (l.source.id === d.id && l.source.type === d.type && l.target.id === n.id && l.target.type === n.type) ||
      ...
      .on("mouseout", (event: MouseEvent, d: NodeType) => {
        tooltipDiv.transition()
          .duration(500)
          .style("opacity", 0);
    ...
    node.each(function (d) {
      const hasNegativeCombination = combinationLinks.some(link =>
        ((link.source.id === d.id && link.source.type === d.type) ||
  ...
  return (
    <div className="flex-grow  rounded-lg shadow overflow-hidden">
      <svg ref={svgRef}></svg>
    </div>
  )
};
